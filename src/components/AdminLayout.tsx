        ...
        <div className="flex-1 flex flex-col">
          <header className="h-12 md:h-14 border-b bg-card flex items-center px-2 sm:px-4 gap-1.5 sm:gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-sm md:text-lg font-semibold truncate">{t('admin.panel')}</h1>
              <p className="text-[10px] md:text-xs text-muted-foreground truncate hidden sm:block">{user?.email}</p>
            </div>
            <div className="flex gap-1.5 md:gap-2">
              <LanguageSwitcher variant="ghost" />
              <NotificationBell />
              <Button variant="ghost" size="icon" onClick={() => navigate('/my-profile')} className="h-7 w-7 md:h-8 md:w-8" aria-label="My Profile">
